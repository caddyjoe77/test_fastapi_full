...
- 🎯 Entry points to execute your program using `python -m <test_fastapi_full>` or `$ test_fastapi_full` with basic CLI argument parsing.
...
---
# test_fastapi_full Flask Application

Awesome test_fastapi_full created by caddyjoe77
...
```bash
git clone https://github.com/caddyjoe77/test_fastapi_full test_fastapi_full
cd test_fastapi_full
make install
```
...
```bash
test_fastapi_full create-db   # run once
test_fastapi_full populate-db  # run once (optional)
test_fastapi_full add-user -u admin -p 1234  # ads a user
test_fastapi_full run
```
