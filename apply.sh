...
original_author="caddyjoe77"
original_name="test_fastapi_full"
original_urlname="test_fastapi_full"
original_description="Awesome test_fastapi_full created by caddyjoe77" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/test_fastapi_full "${name}"
...
then
    ...
    echo "then run 'test_fastapi_full' or 'python -m test_fastapi_full'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'test_fastapi_full' or 'python -m test_fastapi_full' or 'poetry run test_fastapi_full'"
fi
